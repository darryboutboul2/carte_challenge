...
export default function HomeScreen() {
  // --- hooks d’authentification et données fitness
  const { isAdminMode, currentAdmin, currentMember, switchToMemberMode, isLoading: authLoading } = useAuth();
  const {
  ...
  // --- 1) LOGIN MEMBRE via fitness-loyalty (plus de branche admin ici)
  const handleMemberLogin = async (name: string) => {
    if (isLoggingIn) return;
    setIsLoggingIn(true);
    try {
      await fitnessLogin(name.trim());
      router.replace('/(tabs)');  // on force la navigation dès que ça marche
    } catch (err) {
      Alert.alert(
        'Erreur de connexion',
  ...
  const handleValidateVisit = async () => {
    ...
    try {
      ...
      if (result) {
        const msg = getMotivationMessage(result.member.visits, result.member.totalRewards);
        if (msg) {
  ...
  // --- affichage loader si on attend l’auth ou les données loyalty
  ...
  // --- écran de login (toujours keyboardShouldPersistTaps pour capter le premier clic)
  if (!activeMember) {
    return (
      <KeyboardAvoidingView behavior="padding" style={styles.wrapper}>
        ...
        >
          <MemberLogin
            ...
            onBackToAdmin={isAdminMode && currentAdmin ? switchToMemberMode : undefined}
        ...
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.header}>
            ...
            <Text style={styles.headerSubtitle}>
              Prêt pour votre entraînement chez {currentAdmin?.gymName || 'Carte Challenge'} ?
            </Text>
          </View>
            ...
            <LinearGradient colors={['rgba(255,255,255,0.1)', 'rgba(255,255,255,0.05)']} style={styles.cardGradient}>
              <View style={styles.cardHeader}>
                <View style={styles.memberInfo}>
                  ...
                  <View style={styles.levelBadge}>
                    <Text style={styles.levelText}>{activeMember.level}</Text>
                  </View>
                </View>
              ...
              <View style={styles.statsContainer}>
                <View style={styles.statItem}>
                  <Text style={styles.statNumber}>{activeMember.visits}</Text>
                  <Text style={styles.statLabel}>Passages</Text>
                </View>
                <View style={styles.statItem}>
                  <Text style={styles.statNumber}>{activeMember.totalRewards}</Text>
                  <Text style={styles.statLabel}>Récompenses</Text>
                </View>
                <View style={styles.statItem}>
                  <Text style={styles.statNumber}>{Math.floor(activeMember.visits / 7)}</Text>
          ...
          <View style={styles.progressSection}>
            <Text style={styles.sectionTitle}>Progression vers la prochaine récompense</Text>
            <View style={styles.progressCard}>
              <View style={styles.progressHeader}>
                <Text style={styles.progressText}>{progress.current} / {progress.required} passages</Text>
              ...
              <Text style={styles.progressSubtext}>
                Plus que {progress.required - progress.current} passage{progress.required - progress.current > 1 ? 's' : ''} !
              </Text>
            </View>
          </View>

          <VisitButton onValidateVisit={handleValidateVisit} isProcessing={isProcessingVisit} />
...
const styles = StyleSheet.create({
  ...
  loadingText: { fontSize: 18, color: 'white', marginTop: 8 },
  ...
  memberInfo: { flex: 1 },
  memberName: { fontSize: 20, fontWeight: 'bold', color: 'white', marginBottom: 8 },
  levelBadge: { alignSelf: 'flex-start', borderRadius: 20, overflow: 'hidden', paddingHorizontal: 12, paddingVertical: 6, backgroundColor: 'rgba(255,255,255,0.1)' },
  levelText: { fontSize: 14, fontWeight: '600', color: 'white' },
  statsContainer: { flexDirection: 'row', justifyContent: 'space-around' },
  statItem: { alignItems: 'center' },
  statNumber: { fontSize: 24, fontWeight: 'bold', color: 'white', marginTop: 8 },
  statLabel: { fontSize: 12, color: 'rgba(255,255,255,0.7)', textAlign: 'center', marginTop: 4 },
  progressSection: { marginHorizontal: 24, marginBottom: 8 },
  sectionTitle: { fontSize: 18, fontWeight: '600', color: 'white', marginBottom: 16 },
  progressCard: { backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  progressHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 },
  progressText: { fontSize: 16, fontWeight: '600', color: 'white' },
  progressPercentage: { fontSize: 16, fontWeight: 'bold', color: '#fbbf24' },
  progressBarContainer: { height: 8, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 4, marginBottom: 8 },
  ...
  activityItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.1)', padding: 12, borderRadius: 8, marginBottom: 8, gap: 8 },
  activityText: { fontSize: 14, color: 'rgba(255,255,255,0.8)' },
  noActivityText: { fontSize: 14, color: 'rgba(255,255,255,0.6)', textAlign: 'center', padding: 20 },
});
